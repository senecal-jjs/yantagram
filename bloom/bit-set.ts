...
/**
 * A Copy of Callidon/bloom-filters, modified to avoid using base64-arraybuffer
 */
...
export default class BitSet {
  ...
  public export(): { size: number; content: string } {
    return {
      size: this.size,
      content: Base64String.fromBytes(this.array).getValue(),
    };
  }
  ...
  public static import(data: { size: number; content: string }): BitSet {
    ...
    const result = new BitSet(data.size),
      buffer = Base64String.fromBase64(data.content).toBytes();
