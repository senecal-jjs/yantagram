...
import { ExportedBigInt, HashableInput } from "./types";
  ...
  implements WritableFilter<HashableInput>
{
  ...
  constructor(size: number, nbHashes: number) {
    ...
    if (nbHashes < 1) {
      throw new Error(
        `A CountingBloomFilter must use at least one hash function, but you tried to use ${nbHashes} functions. Consider increasing it.`,
      );
    }
